...
struct programRunFlags{
    bool msglvl:2 ; // 0=quiet 1=default 2+ =verbose
};

static programRunFlags program_flags = {1};

#define IF_NQ(...) if(program_flags.msglvl > 0) { __VA_ARGS__ }
#define IF_VB(...) if(program_flags.msglvl > 1) { __VA_ARGS__ }
...
#define HANDLE_DEFAULT_ARGS {              \
    ...
    if (parseArg(argc, argv, "-v") >= 0) {  \
        program_flags.msglvl = 2;           \
    }                                       \
    if (parseArg(argc, argv, "-q") >= 0) {  \
        program_flags.msglvl = 0;           \
    }                                       \
}
